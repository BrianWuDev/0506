...
class MultiTumorNetworkVisualizer:
    ...
    def create_network(self, tumor_data: Dict[str, pd.DataFrame]) -> Network:
        ...
        # 收集所有基因及其在哪些腫瘤中出現
        all_genes = {}  # 格式: {gene_id: {tumor_type: pcc, ...}, ...}
        
        # 第一步：收集所有基因及其在不同腫瘤中的PCC值
        ...
        logger.info(f"發現 {len(cross_tumor_genes)} 個跨腫瘤類型的基因")
        
        # 添加腫瘤節點
        tumor_positions = {}  # 儲存腫瘤節點的位置
        ...
        for tumor_type in self.tumor_types:
            ...
            logger.info(f"{tumor_type}: 添加 {len(tumor_specific_genes)} 個腫瘤特異性基因")
        ...
        for gene_id, tumor_pccs in cross_tumor_genes.items():
            ...
            # 多種顏色混合，創建獨特的節點顏色
            combined_color = '#666666'  # 預設顏色
            tumors_str = ', '.join(tumor_pccs.keys())
            
            tooltip = f"{gene_id}<br>跨腫瘤類型基因<br>存在於: {tumors_str}<br>"
            ...
            logger.info(f"  {gene_id}: 存在於 {len(tumor_pccs)} 種腫瘤類型 ({', '.join(tumor_pccs.keys())})")
        ...
        print(f"總基因數: {len(all_genes)}")
        print(f"腫瘤特異性基因: {tumor_specific_count}")
        print(f"跨腫瘤類型基因: {multi_tumor_count}")
        
        for tumor, count in self.gene_counts.items():
            logger.info(f"  {tumor}: {count} 個基因")
            print(f"  {tumor}: {count} 個基因")
        
        # Configure physics for layout - 調整物理引擎設置以適應更多節點
        physics_options = {
            "enabled": True,
